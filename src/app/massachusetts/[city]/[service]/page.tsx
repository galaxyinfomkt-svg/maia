...
import { getRichParagraphs, getServiceChecklist, getUniqueFAQs, getCityProfile } from '@/lib/content-engine';
...
export default async function MACityServicePage({ params }: MACityServicePageProps) {
  ...
  const housingTypes = profile.housingTypes;
  ...
  const localBusinessSchema = {
    ...
    // branch, so it references the real organization instead of claiming a
    // premises here — with coordinates that were not this city's anyway — and
    // re-declaring the rating on every page.
    '@type': 'Service',
  ...
  // Was six fixed questions with the city name swapped in — the same six on
  // every one of these pages. The engine draws six from a larger bank in a
  // city-specific order, so neighbouring towns ask different things.
  const faqItems = getUniqueFAQs(serviceSlug, city);
  ...
  return (
    <>
      ...
      <HeroWithForm
        ...
        badge={`${city.name}, MA • 5.0★ Rated • Licensed & Insured`}
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            <div>
              ...
              {/* Rich Content: Comprehensive Services */}
              <div className="mb-12">
                ...
                <div className="bg-slate-900 p-8 rounded-2xl text-white">
                  <h4 className="text-xl font-bold mb-4 text-amber-400">Why {city.name} Trusts {SITE_NAME}</h4>
